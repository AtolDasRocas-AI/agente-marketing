-- Acesso da aplicação: somente Google, restrito às contas autorizadas da ATOL.
...
begin
  if v_provider <> 'google' or v_email not in ('atoldasrocas.ai@gmail.com', 'lipe.kosse@gmail.com') then
    return pg_catalog.jsonb_build_object('error', pg_catalog.jsonb_build_object(
      'http_code', 403,
      'message', 'O acesso é restrito às contas Google autorizadas da ATOL.'
    ));
  end if;
  return '{}'::jsonb;
end;
