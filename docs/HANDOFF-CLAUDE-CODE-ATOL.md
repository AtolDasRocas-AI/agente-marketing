...
- **Onda 2 (insights de post + conta):** `marketing-importar-metricas-instagram` passou a chamar `/insights` por mídia e `/insights` de conta + `followers_count`. Nova tabela `0031_marketing_instagram_account_daily.sql` (série diária, upsert por dia). Cron novo `importar-metricas-marketing` (`0032`, 03:20 UTC). A function aceita chamada via `x-cron-secret` além de sessão de usuário (implantada com `--no-verify-jwt`, mesmo padrão de `lgpd-expurgo-marketing`). **Stories ficou fora** — investigado ao vivo e descartado em definitivo (ver seção própria abaixo). **Bug encontrado em produção e corrigido em 16/09/2026 — ver bullet próprio abaixo.**
...
**Bug real de produção encontrado e corrigido (16/09/2026):** o responsável abriu a tela de Métricas em produção e viu alcance/visualizações/salvamentos/compartilhamentos como "não disponível" num post com curtidas/comentários normais (34/26) — sinal de que não era só dado ainda não coletado. Causa raiz: a Onda 2 chaveava a matriz de insight por `media_product_type` (FEED/REELS/STORY), campo pedido no `/media`. A documentação do objeto de mídia (`developers.facebook.com/documentation/instagram-platform/reference/instagram-media`, não checada na validação original da onda) diz que esse campo está "disponível apenas para a API do Instagram com o Login do Facebook" — não o Login do Instagram que este projeto usa. O campo nunca vinha preenchido, e a busca de insight nunca era sequer tentada, para nenhum post, desde o deploy da Onda 2. Corrigido: `marketing-importar-metricas-instagram` agora escolhe a matriz por `media_type` (IMAGE/VIDEO/CAROUSEL_ALBUM), sem essa restrição; `media_product_type` removido do `fields=`. Reimplantado. **Posts já importados antes da correção continuam com esses campos vazios até serem reimportados** — clicar "Importar métricas agora" (ou esperar o cron das 03:20 UTC) resolve.
